...
const BuyerDashboard = () => {
  ...
  const getStatusCounts = () => {
    // Since we load data per filter, we just show the current count
    return {
      active: statusFilter === 'active' ? auctions.length : 0,
      completed: statusFilter === 'completed' ? auctions.length : 0
    };
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900">Browse Auctions</h2>
        <Gavel className="w-8 h-8 text-blue-600" />
      </div>
